...
def get_Andrew_Huberman_Insights(query:str) -> str:
    ...
    key = __get_pinecone_api_key()

    try:
        # Initialize Pinecone client
        pc = Pinecone(api_key = key)
        index = pc.Index(host = "https://ourahuberman-2qajcgl.svc.aped-4627-b74a.pinecone.io")
...
def __get_device_api_key(user_id: str, device_type: str):
    """
    Get the API key for the user
    """
    with open(MAIN_DB_PATH, 'r') as file:
        user_db = json.load(file)
        # Need to filter an array of dicts by device_type - expecting low device count
        api_key = [device["api_key"] for device in user_db[user_id]["devices"] if device["device_type"]== device_type]
        return api_key[0] # assumption is that there is only one device per user
    
# Get the Pinecone API key
# Returns: str - the Pinecone API key
def __get_pinecone_api_key():
    """
    Get the Pinecone API key
    """
