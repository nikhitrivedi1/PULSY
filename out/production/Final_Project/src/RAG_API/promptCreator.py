...
class Prompt:
    def __init__(self):
        ...
        self.prompt = PromptTemplate(template = self.template, input_variables = ["question", "documents", "userData", "descriptions"])

    def getPrompt(self, question, documents, ouraData, descriptions):
        ouraDataForm = self.getOuraUserData(ouraData, question)
        print(ouraDataForm)
        return self.prompt.format(question  = question, documents = documents, userData = ouraDataForm, descriptions = descriptions)
